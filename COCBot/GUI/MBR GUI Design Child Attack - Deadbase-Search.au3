...
Local $x = 25, $y = 45
	...
	$x = 10
		...
		$txtTip = "Check To make the bot waits for spells" & @CRLF & _
		"It will also automatically check Take spells into account in Doc Oc Tab"
 		GUICtrlSetTip(-1, $txtTip)
...
Local $x = 220, $y = 45
	$grpDBFilter = GUICtrlCreateGroup(GetTranslated(625,14, "Filters"), $x - 20, $y - 20, 225, 305)
		...
		$y += 25
		$chkDBMeetCollOutside = GUICtrlCreateCheckbox("Exposed collectors", $x, $y, -1, -1)
			$txtTip = "Search villages with exposed collectors."
		...
		$txtDBMinCollOutsidePercent = GUICtrlCreateInput("50", $x + 143, $y, 31, 20, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
			$txtTip = "Minimum percentage of exposed collectors outside the red zone."
			GUICtrlSetTip(-1, $txtTip)
			GUICtrlSetState(-1, $GUI_DISABLE)
			GUICtrlSetLimit(-1, 3)
			GUICtrlSetData(-1, 50)
