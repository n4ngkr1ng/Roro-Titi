...
;~ 	IniWriteS($config, "donate", "chkRequest", $iChkRequest)
	...
	;Telegram[Surbiks]
	IniWriteS($config, "telegram", "AccountToken", $TelegramToken)
	IniWriteS($config, "telegram", "OrigTelegram", $iOrigTelegram)
	IniWriteS($config, "telegram", "TelegramEnabled", $TelegramEnabled)
	IniWriteS($config, "telegram", "TelegramRemote", $pRemoteTelegram)
	IniWriteS($config, "telegram", "DeleteAllTelegramPushes", $iDeleteAllTPushes)
	IniWriteS($config, "telegram", "HoursTelegram", $icmbHoursTelegram)
	IniWriteS($config, "telegram", "AlertTelegramVMFound", $pMatchFoundTelegram)
	IniWriteS($config, "telegram", "AlertTelegramLastRaid", $pLastRaidImgTelegram)
	IniWriteS($config, "telegram", "AlertTelegramWallUpgrade", $pWallUpgradeTelegram)
	IniWriteS($config, "telegram", "AlertTelegramOOS", $pOOSTelegram)
	IniWriteS($config, "telegram", "AlertTelegramOtherDevice", $pAnotherDeviceTelegram)
	IniWriteS($config, "telegram", "AlertTelegramLastRaidTxt", $iAlertTLastRaidTxt)
	IniWriteS($config, "telegram", "AlertTelegramCampFull", $ichkAlertTCampFull)
	IniWriteS($config, "telegram", "AlertTelegramVillage", $iAlertTVillage)
	IniWriteS($config, "telegram", "AlertTelegramLastAttack", $iLastAttackT)
	IniWriteS($config, "telegram", "AlertTelegramVBreak", $pTakeAbreakTelegram)

	IniWriteS($config, "telegram", "TelegramChatIDs", _ArrayToString($TelegramChatIDs))
	IniWriteS($config, "telegram", "TelegramLastRemoteID", $TelegramLastRemoteID)
	;Telegram
	...
	If GUICtrlRead($chkCSVSpeedStaticDB) = $GUI_CHECKED Then
		IniWrite($config, "attack", "chkCSVSpeedStaticDB", 1)
	Else
		IniWrite($config, "attack", "chkCSVSpeedStaticDB", 0)
	EndIf

	If GUICtrlRead($chkCSVSpeedVariableDB) = $GUI_CHECKED Then
		IniWrite($config, "attack", "ichkCSVSpeedVariableDB", 1)
	Else
		IniWrite($config, "attack", "ichkCSVSpeedVariableDB", 0)
	EndIf

	IniWriteS($config, "attack", "CSVSpeedMinDB", $isldSelectedCSVSpeedMin[$DB])
	IniWriteS($config, "attack", "CSVSpeedMaxDB", $isldSelectedCSVSpeedMax[$DB])

	If GUICtrlRead($chkCSVSpeedStaticAB) = $GUI_CHECKED Then
		IniWrite($config, "attack", "chkCSVSpeedStaticAB", 1)
	Else
		IniWrite($config, "attack", "chkCSVSpeedStaticAB", 0)
	EndIf

	If GUICtrlRead($chkCSVSpeedVariableAB) = $GUI_CHECKED Then
		IniWrite($config, "attack", "ichkCSVSpeedVariableAB", 1)
	Else
		IniWrite($config, "attack", "ichkCSVSpeedVariableAB", 0)
	EndIf

	IniWriteS($config, "attack", "CSVSpeedMinAB", $isldSelectedCSVSpeedMin[$LB])
	IniWriteS($config, "attack", "CSVSpeedMaxAB", $isldSelectedCSVSpeedMax[$LB])
...
;~ 	IniWriteS($config, "MilkingAttack", "MASmartAttackDarkElixirDrill", $iChkSmartAttack[$MA][2])
	...
	If GUICtrlRead($chkUseSpellsTrainingClose) = $GUI_CHECKED Then
		IniWrite($config, "Close When Training", "CloseWhenTrainingSpells", 1)
	Else
		IniWrite($config, "Close When Training", "CloseWhenTrainingSpells", 0)
	EndIf
	...
	;Train Big First
	If GUICtrlRead ($ChktrainBigFirst) = $GUI_CHECKED Then
	   IniWrite($config, "troop", "ChktrainBigFirst", 1)
    Else
	   IniWrite($config, "troop", "ChktrainBigFirst", 0)
    EndIf
