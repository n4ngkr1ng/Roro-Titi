...
;~ 	EndIf
	...
	If $ichkCSVSpeedStaticDB = 1 Then
		GUICtrlSetState($chkCSVSpeedStaticDB, $GUI_CHECKED)
 	Else
		GUICtrlSetState($chkCSVSpeedStaticDB, $GUI_UNCHECKED)
	EndIf
	chkCSVSpeedStaticDB()

	If $ichkCSVSpeedVariableDB = 1 Then
		GUICtrlSetState($chkCSVSpeedVariableDB, $GUI_CHECKED)
 	Else
		GUICtrlSetState($chkCSVSpeedVariableDB, $GUI_UNCHECKED)
	EndIf
	chkCSVSpeedVariableDB()

	GUICtrlSetData($sldSelectedSpeedDB, $isldSelectedCSVSpeed[$DB])
	GUICtrlSetData($sldSelectedSpeedMinDB, $isldSelectedCSVSpeedMin[$DB])
	sldSelectedSpeedMinDB()
	GUICtrlSetData($sldSelectedSpeedMaxDB, $isldSelectedCSVSpeedMax[$DB])
	sldSelectedSpeedMaxDB()

	If $ichkCSVSpeedStaticAB = 1 Then
		GUICtrlSetState($chkCSVSpeedStaticAB, $GUI_CHECKED)
 	Else
		GUICtrlSetState($chkCSVSpeedStaticAB, $GUI_UNCHECKED)
	EndIf
	chkCSVSpeedStaticAB()

	If $ichkCSVSpeedVariableAB = 1 Then
		GUICtrlSetState($chkCSVSpeedVariableAB, $GUI_CHECKED)
 	Else
		GUICtrlSetState($chkCSVSpeedVariableAB, $GUI_UNCHECKED)
	EndIf
	chkCSVSpeedVariableAB()

	GUICtrlSetData($sldSelectedSpeedAB, $isldSelectedCSVSpeed[$LB])
	GUICtrlSetData($sldSelectedSpeedMinAB, $isldSelectedCSVSpeedMin[$LB])
	sldSelectedSpeedMinAB()
	GUICtrlSetData($sldSelectedSpeedMaxAB, $isldSelectedCSVSpeedMax[$LB])
	sldSelectedSpeedMaxAB()
	...
	If $chkLvl10Enabled = "1" Then
		...
		For $i = $cmbLvl10 To $txtLvl10Full
		   GUICtrlSetState($i, $GUI_SHOW)
		Next
	Else
	...
	If $chkLvl11Enabled = "1" Then
		...
		For $i = $cmbLvl11 To $txtLvl11Full
		   GUICtrlSetState($i, $GUI_SHOW)
		Next
	Else
	...
	If $chkLvl12Enabled = "1" Then
		...
		For $i = $cmbLvl12 To $txtLvl12Full
		   GUICtrlSetState($i, $GUI_SHOW)
		Next
	Else
...
_GUICtrlComboBox_SetCurSel($cmbTHSnipeBeforeLBScript, _GUICtrlComboBox_FindStringExact($cmbTHSnipeBeforeLBScript, $THSnipeBeforeLBScript))
	...
	If $ichkSmartZap = 1 Then
		...
		For $i = $chkSmartZapDB To $lblLightningUsed
		   GUICtrlSetState($i, $GUI_SHOW)
		Next
	Else
	...
	If $ichkUseSpellsTrainingClose = 1 Then
  ...
  chkABSpellsWait()
	...
	;Train Big First
	If $iChktrainBigFirst = 1 Then
	   GUICtrlSetState ($ChktrainBigFirst, $GUI_CHECKED)
    Else
	   GUICtrlSetState($ChktrainBigFirst, $GUI_UNCHECKED)
    EndIf
	...
	If $ichkSwitchAcc = 1 Then
	   ...
	   GUICtrlSetState($chkUseTrainingClose, $GUI_DISABLE)
		For $i = $chkUseSpellsTrainingClose To $chkRandomStayORClose
			GUICtrlSetState($i, $GUI_HIDE)
		Next
		For $i = $lbTotalCoCAcc To $HelpSwich
			GUICtrlSetState($i, $GUI_SHOW)
		Next
    Else
