...
Func readConfig($inputfile = $config, $partial = False) ;Reads config and sets it to the variables
	...
	If FileExists($config) Then
		...
		IniReadS($ichkBotStop, $config, "general", "BotStop", "1")
		IniReadS($icmbBotCommand, $config, "general", "Command", "7")
		IniReadS($icmbBotCond, $config, "general", "Cond", "22")
	...
	$iHasteSpellBrewInAdvance = Int(IniRead($config, "Spells", "BrewInAdvanceHasteSpell", "0"))
		...
		IniReadS($iChkSmartAttack[$DB][0], $config, "attack", "DBSmartAttackGoldMine", "1")
		IniReadS($iChkSmartAttack[$DB][1], $config, "attack", "DBSmartAttackElixirCollector", "1")
		IniReadS($iChkSmartAttack[$DB][2], $config, "attack", "DBSmartAttackDarkElixirDrill", "1")
		...
		IniReadS($iChkSmartAttack[$LB][0], $config, "attack", "ABSmartAttackGoldMine", "1")
		IniReadS($iChkSmartAttack[$LB][1], $config, "attack", "ABSmartAttackElixirCollector", "1")
		IniReadS($iChkSmartAttack[$LB][2], $config, "attack", "ABSmartAttackDarkElixirDrill", "1")
		...
		IniReadS($ichkCSVSpeedStaticDB, $config, "attack", "chkCSVSpeedStaticDB", 0)
		IniReadS($ichkCSVSpeedVariableDB, $config, "attack", "chkCSVSpeedVariableDB", 1)
		IniReadS($isldSelectedCSVSpeedMin[$DB], $config, "attack", "CSVSpeedMinDB", 3)
		IniReadS($isldSelectedCSVSpeedMax[$DB], $config, "attack", "CSVSpeedMaxDB", 6)

		IniReadS($ichkCSVSpeedStaticAB, $config, "attack", "chkCSVSpeedStaticAB", 0)
		IniReadS($ichkCSVSpeedVariableAB, $config, "attack", "chkCSVSpeedVariableAB", 1)
		IniReadS($isldSelectedCSVSpeedMin[$LB], $config, "attack", "CSVSpeedMinAB", 3)
		IniReadS($isldSelectedCSVSpeedMax[$LB], $config, "attack", "CSVSpeedMaxAB", 6)
...
;~ 		IniReadS($iChkSmartAttack[$MA][2], $config, "MilkingAttack", "MASmartAttackDarkElixirDrill", "0")
		...
		IniReadS($iChkUseCCBalanced, $config, "ClanClastle", "BalanceCC", "1")
		...
		IniReadS($ichkCleanYard, $config, "other", "chkCleanYard", "1")
		;Boju Only clear GemBox
		IniReadS($ichkGemsBox, $config, "other", "chkGemsBox", "0")
		;Only clear GemBox
		IniReadS($sTimeWakeUp, $config, "other", "txtTimeWakeUp", "0")
		IniReadS($iVSDelay, $config, "other", "VSDelay", "2")
		IniReadS($iMaxVSDelay, $config, "other", "MaxVSDelay", "6")
		...
		$minTrainAddition = Number(IniRead($config, "Close When Training", "AdditionMin", "2"))
		$maxTrainAddition = Number(IniRead($config, "Close When Training", "AdditionMax", "12"))
		$LeaveCoCOpen = IniRead($config, "Leave CoC Open", "Enabled", "0")
		$CloseCoCGame = IniRead($config, "Close CoC Game", "Enabled", "0")
		$RandomCoCOpen = IniRead($config, "Random Leave-Close", "Enabled", "1")
		$RandomCloseTraining = IniRead($config, "Random Stay-Close Game", "Enabled", "1")
		$ichkUseSpellsTrainingClose =  IniRead($config, "Close When Training", "CloseWhenTrainingSpells", "0")
		...
		$ichkLimitAttacks = Number(IniRead($config, "Daily Attacks", "Enabled", "0"))
		...
		$ichkCloseNight = Number(IniRead($config, "Simulate Sleep", "Enabled", "0"))
		...
		;Train Big First
		$iChktrainBigFirst = IniRead($config, "troop", "ChktrainBigFirst", "0")
