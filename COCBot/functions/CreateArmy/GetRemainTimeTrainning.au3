...
Func getRemainingTraining($Troops = True, $Spells = True)
	...
	If openArmyOverview() Then
		...
		Local $ResultSpells = getRemainTrainingTimer(360, 422)

		If $Troops = True Then
			SetLog("Troops training duration : " & $ResultTroops, $COLOR_PURPLE)
		...
		If $Spells = True Then
			SetLog("Spells brewing duration : " & $ResultSpells, $COLOR_PURPLE)
