...
Func AreCollectorsOutside($percent) ; dark drills are ignored since they can be zapped
	SetLog("Localising Mines & Collectors", $COLOR_BLUE)
	; reset variables
	Global $PixelMine[0]
	...
	SetLog("Localised in " & Round(TimerDiff($hTimer) / 1000, 2) & " seconds:")
	SetLog("[" & UBound($PixelMine) & "] Mines")
	SetLog("[" & UBound($PixelElixir) & "] Collectors")
