...
Func ParseAttackCSV($debug = False)
	...
	If ($ichkCSVSpeedStaticDB = 1 And $ichkCSVSpeedVariableDB = 0) And $iMatchMode = $DB Then
	$ResultCSVSpeedDeploy = $iCSVSpeeds[$isldSelectedCSVSpeed[$iMatchMode]]
	ElseIf ($ichkCSVSpeedStaticDB = 0 And $ichkCSVSpeedVariableDB = 1) And $iMatchMode = $DB Then
	$ResultCSVSpeedDeploy = Random($iCSVSpeeds[$isldSelectedCSVSpeedMin[$iMatchMode]], $iCSVSpeeds[$isldSelectedCSVSpeedMax[$iMatchMode]])
	ElseIf ($ichkCSVSpeedStaticAB = 1 And $ichkCSVSpeedVariableAB = 0) And $iMatchMode = $LB Then
	$ResultCSVSpeedDeploy = $iCSVSpeeds[$isldSelectedCSVSpeed[$iMatchMode]]
	ElseIf ($ichkCSVSpeedStaticAB = 0 And $ichkCSVSpeedVariableAB = 1) And $iMatchMode = $LB Then
	$ResultCSVSpeedDeploy = Random($iCSVSpeeds[$isldSelectedCSVSpeedMin[$iMatchMode]], $iCSVSpeeds[$isldSelectedCSVSpeedMax[$iMatchMode]])
	EndIf
	$TxtSpeedDeploy = Round($ResultCSVSpeedDeploy, 2)
	...
	Setlog("CSV Deployment Speed : " & $TxtSpeedDeploy & " X", $COLOR_BLUE)
