...
Global $aTabControlsDocOc[4] = [$hGUI_DOCOC_TAB, $hGUI_DOCOC_TAB_ITEM1, $hGUI_DOCOC_TAB_ITEM2, $hGUI_DOCOC_TAB_ITEM3]
Global $aTabControlsBattleSettings[2] = [$hGUI_BattleSettings_TAB, $hGUI_BattleSettings_TAB_ITEM1]
Global $aTabControlsDivers[2] = [$hGUI_Divers_TAB, $hGUI_Divers_TAB_ITEM1]
Global $aTabForecast[2] = [$hGUI_METEO_TAB, $hGUI_METEO_TAB_ITEM1]

Global $aTabControlsBot[6] = [$hGUI_BOT_TAB, $hGUI_BOT_TAB_ITEM1, $hGUI_BOT_TAB_ITEM4, $hGUI_BOT_TAB_ITEM2, $hGUI_BOT_TAB_ITEM3]
...
Func IsTab($controlID)
	If _ArraySearch($aMainTabItems, $controlID) <> -1 Or _
			_ArraySearch($aTabControlsBattleSettings, $controlID) <> -1 Or _
			_ArraySearch($aTabControlsDivers, $controlID) <> -1 Or _
			...
			_ArraySearch($aTabControlsDocOc, $controlID) <> -1 Or _
			_ArraySearch($aTabControlsBot, $controlID) <> -1 Or _
			_ArraySearch($aTabForecast, $controlID) <> -1 Or _
...
#include "GUI\MBR GUI Control Child DocOc.au3"
			...
			Case $tabidx = 0 ; Deadbase tab
				...
				Else
					...
					If GUICtrlRead($chkABSpellsWait) = $GUI_CHECKED Then
						GUICtrlSetState($chkUseSpellsTrainingClose, $GUI_CHECKED)
						GUICtrlSetState($chkUseSpellsTrainingClose, $GUI_DISABLE)
					Else
						GUICtrlSetState($chkUseSpellsTrainingClose, $GUI_UNCHECKED)
						GUICtrlSetState($chkUseSpellsTrainingClose, $GUI_ENABLE)
					EndIf
				EndIf
...
Func tabMod()
	$tabidx = GUICtrlRead($hGUI_MOD_TAB)
		Select
			Case $tabidx = 0 ;  tab
				GUISetState(@SW_SHOWNOACTIVATE, $hGUI_DOCOC)
				GUISetState(@SW_HIDE, $hGUI_BattleSettings)
				GUISetState(@SW_HIDE, $hGUI_Divers)
				GUISetState(@SW_HIDE, $hGUI_METEO)
			Case $tabidx = 1 ;  tab
				GUISetState(@SW_HIDE, $hGUI_DOCOC)
				GUISetState(@SW_SHOWNOACTIVATE, $hGUI_BattleSettings)
				GUISetState(@SW_HIDE, $hGUI_Divers)
				GUISetState(@SW_HIDE, $hGUI_METEO)
			Case $tabidx = 2 ;  tab
				GUISetState(@SW_HIDE, $hGUI_DOCOC)
				GUISetState(@SW_HIDE, $hGUI_BattleSettings)
				GUISetState(@SW_SHOWNOACTIVATE, $hGUI_Divers)
				GUISetState(@SW_HIDE, $hGUI_METEO)
			Case $tabidx = 3
				GUISetState(@SW_HIDE, $hGUI_DOCOC)
				GUISetState(@SW_HIDE, $hGUI_BattleSettings)
				GUISetState(@SW_HIDE, $hGUI_Divers)
				GUISetState(@SW_SHOWNOACTIVATE, $hGUI_METEO)
			Case ELSE
				GUISetState(@SW_HIDE, $hGUI_DOCOC)
				GUISetState(@SW_HIDE, $hGUI_BattleSettings)
				GUISetState(@SW_HIDE, $hGUI_Divers)
...
Func tabDocOc()
	$tabidx = GUICtrlRead($hGUI_DOCOC_TAB)
		Select
			Case $tabidx = 0
				GUISetState(@SW_SHOWNOACTIVATE, $hGUI_DOCOC_TAB_ITEM1)
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM2)
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM3)
			Case $tabidx = 1
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM1)
				GUISetState(@SW_SHOWNOACTIVATE, $hGUI_DOCOC_TAB_ITEM2)
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM3)
			Case $tabidx = 2
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM1)
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM2)
				GUISetState(@SW_SHOWNOACTIVATE, $hGUI_DOCOC_TAB_ITEM3)
			Case ELSE
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM1)
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM2)
				GUISetState(@SW_HIDE, $hGUI_DOCOC_TAB_ITEM3)
...
Func Bind_ImageList($nCtrl)
	...
	Switch $nCtrl
		Case $tabMain
			...
			Local $aIconIndex[6] = [$eIcnHourGlass, $eIcnTH11, $eIcnCamp, $eIcnGUI, $eIcnCrown, $eIcnInfo] ; Modified by CDudz

		Case $hGUI_VILLAGE_TAB
			; the icons for village tab
			Local $aIconIndex[5] = [$eIcnTH1, $eIcnCC, $eIcnLaboratory, $eIcnAchievements, $eIcnNotification]
		...
		Case $hGUI_NOTIFY_TAB
			...
			;Telegram[Surbiks]
			Local $aIconIndex[4] = [$eIcnPushBullet, $eIcnInfo, $eIcnTelegram, $eIcnInfo]
			;Telegram
		...
		Case $hGUI_BOT_TAB
			...
			Local $aIconIndex[5] = [$eIcnOptions, $eIcnProfile, $eIcnSwitch, $eIcnGold]
		...
		Case $hGUI_MOD_TAB
			; the icons for Bot tab
			Local $aIconIndex[4] = [$eIcnPieuvre, $eIcnEpee, $eIcnParam, $eIcnSoleil]

		Case $hGUI_DOCOC_TAB
			; the icons for strategies tab
			Local $aIconIndex[3] = [$eIcnChrono, $eIcnDodo, $eIcnCalendrier]

		Case $hGUI_STATS_TAB
			; the icons for stats tab
			Local $aIconIndex[4] = [$eIcnGoldElixir, $eIcnOptions, $eIcnCamp, $eIcnPeople]
