...
$sModversion = "RoroTiti, nangkring :-) and Moebius14 MOD v0.4 "
...
SetLog("================================================", $COLOR_RED)          ; Message Log at start
SetLog(" ", $COLOR_RED)                                                         ; Message Log at start
SetLog("          Welcome to the RoroTiti & Moebius14's MOD v0.4 !", $COLOR_RED); Message Log at start
SetLog(" ", $COLOR_RED)                                                         ; Message Log at start
SetLog("================================================", $COLOR_RED)          ; Message Log at start
SetLog(" ", $COLOR_RED)															; Message Log at start
...
ResetTrainTimer()	;chalicucu + demen reset training time
...
;~ 			clanHop()  		    			; Added for Clan Hop mod
			...
			If $RandomCloseTraining = 1 then
				...
				If $RAndomCloseTraining2 = 1 And $bTrainEnabled = True Then
				Setlog("The bot will stay online during troops training...", $COLOR_PURPLE)
				EndIf
			EndIf
...
Func Idle() ;Sequence that runs until Full Army
	...
	While $fullArmy = False Or $bFullArmyHero = False Or $bFullArmySpells = False Or $CommandStop = 0 ;While $fullArmy = False Or $bFullArmyHero = False
		If checkSleep() And $RunState And $ichkCloseNight = 1 Then
			If $debugSetLog = 1 Then SetLog("Sleep Start: " & $nextSleepStart & " - Sleep End: " & $nextSleepEnd, $COLOR_MAROON)
			SetLog("Time to log out for sleep period...", $COLOR_GREEN)
			CloseCOCAndWait(calculateTimeRemaining($nextSleepEnd))
...
Func GotoAttack()
	 If Not $iChkAtkPln Then Return True		;Chalicucu disable attack plan
